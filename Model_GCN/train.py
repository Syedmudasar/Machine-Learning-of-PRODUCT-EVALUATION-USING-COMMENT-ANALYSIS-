...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_string('dataset', 'Amazon', 'Dataset string.')
...
f = open('data/corpus/' + 'Amazon' + '_vocab.txt', 'r')
words = f.readlines()
f.close()
...
word_embeddings_str = '\n'.join(word_vectors)
f = open('data/' + 'Amazon' + '_word_vectors.txt', 'w')
...
doc_embeddings_str = '\n'.join(doc_vectors)
f = open('data/' + 'Amazon' + '_doc_vectors.txt', 'w')
